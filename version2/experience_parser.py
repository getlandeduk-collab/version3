...
def parse_date_string(date_str: str) -> Optional[datetime]:
    ...
    # CRITICAL: Use fixed date for "Present" to ensure deterministic output
    # Using a far future date (2099-12-31) to represent "ongoing" without causing calculation issues
    if date_str.lower() in ["present", "current", "now"]:
        return datetime(2099, 12, 31)  # Fixed date for determinism, not datetime.now()
