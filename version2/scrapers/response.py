...
def summarize_scraped_data(
    ...
    # Use task-specific model selection for summarization
    from model_config import get_model_for_task
    # Summarization uses temperature=0.3 for more natural language
    summarizer_model = get_model_for_task("summarization", json_mode=False)
    model_name = summarizer_model.model_name
    from langchain_openai import ChatOpenAI
    # Create model with task-specific temperature
    model = ChatOpenAI(model=model_name, temperature=0.3)  # 0.3 for more natural summaries
