...
class JobPosting(BaseModel):
    ...
    description: Optional[str] = None  # Raw scraped content (used for scoring)
    job_description: Optional[str] = None  # Cleaned description (for Firestore, internal use only)
...
class Settings(BaseModel):
    """
    Application settings with task-specific model configuration.
    
    Model Configuration:
    - Task-specific models can be set via environment variables:
      * OPENAI_MODEL_RESUME_PARSING (default: gpt-4o-mini)
      * OPENAI_MODEL_JOB_EXTRACTION (default: gpt-4o)
      * OPENAI_MODEL_MATCHING (default: gpt-4o)
      * OPENAI_MODEL_SPONSORSHIP (default: gpt-4o-mini)
      * OPENAI_MODEL_SUMMARIZATION (default: gpt-4o-mini)
      * OPENAI_MODEL_OCR_FALLBACK (default: gpt-4o-mini)
    
    - Critical paths (resume_parsing, matching, job_extraction) are LOCKED
      to deterministic models and cannot be overridden in production.
    
    - Legacy model_name field is kept for backward compatibility but
      task-specific models take precedence.
    """
    model_config = ConfigDict(protected_namespaces=())
    openai_api_key: Optional[str] = None
    firecrawl_api_key: Optional[str] = None
    model_name: str = "gpt-4o"  # Legacy: Use task-specific models instead
