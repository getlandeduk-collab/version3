...
def create_job_matching_graph(
    ...
    # Initialize models using task-specific configuration
    from model_config import get_model_for_task
    parser_model = get_model_for_task("resume_parsing", json_mode=True)
    scraper_model = get_model_for_task("job_extraction", json_mode=False)
    scorer_model = get_model_for_task("matching", json_mode=False)
