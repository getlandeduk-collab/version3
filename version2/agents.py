...
# Import new model configuration system
from model_config import get_model_config as _get_model_config_new, get_model_for_task

def get_model_config(model_name: str, default_temperature: float = 0, json_mode: bool = False) -> ChatOpenAI:
    """
    Get model configuration with temperature support check.
    
    DEPRECATED: Use get_model_for_task() for new code.
    This function is kept for backward compatibility.
    
    Args:
        model_name: Name of the model
        default_temperature: Desired temperature (only used if model supports it)
        json_mode: Whether to enable JSON mode (explicit control)
    
    Returns:
        ChatOpenAI model instance
    """
    return _get_model_config_new(model_name, default_temperature, json_mode)
